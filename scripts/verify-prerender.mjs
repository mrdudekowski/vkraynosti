...
const run = async () => {
  for (const routePath of SAMPLE_ROUTES) {
    ...
    process.stdout.write(`[PASS] prerender meta ${routePath}\n`);
  }
  process.stdout.write('Prerender verification passed.\n');
};

run().catch((error) => {
  process.stderr.write(
    `verify-prerender failed: ${error instanceof Error ? error.message : String(error)}\n`,
  );
  process.exitCode = 1;
});
