...
**Контекст продукта, зафиксированные решения и инженерный backlog по коду (в т.ч. отладочные вызовы в `PageMeta`, маршруты, константы анимаций):** [PROJECT_CONTEXT.md](./PROJECT_CONTEXT.md).
...
1. **Контакты и доверие** — в `src/constants/contacts.ts` телефон и `tel:` сейчас плейсхолдеры (**намеренно на текущем этапе**). Перед публикацией заменить на реальные данные и согласовать с футером/мессенджерами; канонические URL и `SITE_URL` — в финальной правке перед продакшеном (см. [PROJECT_CONTEXT.md](./PROJECT_CONTEXT.md)).
...
- [x] **Баннер согласия на cookies** и условная загрузка Яндекс.Метрики (`CookieConsentProvider`, `VITE_YANDEX_METRIKA_ID`, политика в `UI.privacyPage`).
- [ ] **Аналитика (прод):** ID счётчика в secrets деплоя, проверка визитов в кабинете Метрики, цели конверсии.
...
- 2026-04-17: уточнены намеренные плейсхолдеры и pre-prod URL; аналитика + cookie consent переведены из «опционально» в обязательный трек; добавлена ссылка на [PROJECT_CONTEXT.md](./PROJECT_CONTEXT.md).
