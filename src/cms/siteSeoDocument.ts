...
export const SITE_SEO_PAGE_KEYS = Object.keys(SITE_SEO_PAGE_PATHS) as Array<keyof typeof SITE_SEO_PAGE_PATHS>;

const siteSeoPageSchema = z.object({
  key: z.enum(['home', 'winter', 'spring', 'summer', 'fall']),
  path: z.string().min(1),
...
export const siteSeoDocumentSchema = z.object({
  ...
  const keys = document.pages.map((page) => page.key);
  const uniqueKeys = new Set(keys);
  if (uniqueKeys.size !== SITE_SEO_PAGE_KEYS.length || SITE_SEO_PAGE_KEYS.some((key) => !uniqueKeys.has(key))) {
