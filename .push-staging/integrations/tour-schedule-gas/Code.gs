...
function setupCatalogTourPickLabelsForMenu() {
  ...
  SpreadsheetApp.getUi().alert(
    'Готово: колонка «Подпись списка» (E) на листах «Туры_*» заполняется автоматически.\n' +
      'Вводите A–D (ID, название, цена, тип) и F (статус на сайте).'
  );
}
...
function ensureInstructionSheet_(ss) {
  ...
  var lines = [
    ...
    ['Меню «Вкрайности» вверху справа (если не видно — нажмите 💬). Полная версия: docs/TOUR_DATA_MANAGER_GUIDE.md'],
    [''],
    ['🌲 Добавить тур в сезон'],
    ['   Лист «Туры_Зима» / «Туры_Весна» / «Туры_Лето» / «Туры_Осень»'],
    ['   A = ID (summer-3, winter-1), B = название, C = цена ₽ или пусто, D = однодневный/многодневный'],
    ['   F = статус на сайте: активен | в разработке | скрыт (пусто = черновик)'],
    ['   E («Подпись списка») не трогать — заполняется сама'],
    [''],
    ['📅 Добавить выезд'],
    ['   Лист «Расписание_*» того же сезона'],
    ['   Обязательно: A = дата, C = тур из списка, H = статус (запланирован / набор открыт / мест нет / отменён / завершился)'],
    ['   По желанию: G = места, I = комментарий. Пустой H = черновик'],
    ['   B, D, E, F не редактировать — формулы'],
    [''],
    ['📤 Публикация на сайт (автообновления нет)'],
    ['   «Обновить туры» / «Обновить расписание» / «Обновить всё»'],
    [''],
    ['❌ Проверка: «Проверить таблицу» → лист «_Проверка»'],
    ['   Предупреждение «нет на сайте» — тур не в каталоге сайта (нужен разработчик)'],
    ['   Ошибка — нет даты, неверный статус, дубликат дата+тур и т.д.'],
    [''],
    ['🧟 Чего нельзя: карандаш у списка «Тур»; правка E на Туры_*; правка B,D,E,F на Расписание_*; дубли дата+тур'],
    [''],
    ['💡 #ERROR!: «Исправить формулы» или «Настроить таблицу (всё)»; локаль Файл→Настройки = Россия'],
  ];
...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addSeparator()
    .addItem('Опубликовать всё и пересобрать сайт (SEO)', 'publishAllJsonAndRebuildForMenu')
    .addItem('Пересобрать сайт (SEO)', 'rebuildSiteForMenu')
    .addSeparator()
