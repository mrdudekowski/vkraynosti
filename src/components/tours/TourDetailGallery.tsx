...
const TourDetailGalleryComponent = ({
  ...
  const imgClassesForWinterFraming = (imageSrc: string) => {
    ...
    if (imageSrc === TOUR_SPRING_13_VIEW7_IMAGE) {
      return 'object-gallery-gamova-view7';
    }
  ...
  const videoObjectClassesForGallery = (assetSrc: string) => {
    if (assetSrc === TOUR_SUMMER_1_CLIP4_GRID_WEBM) {
      return TOUR_SUMMER_1_CLIP4_GRID_VIDEO_OBJECT_CLASS;
    }
    return '';
  };
  ...
  const renderTileButton = (
    ...
    if (isVideoAssetUrl(src)) {
      ...
      const videoObjectClassName = videoObjectClassesForGallery(src);
      return (
        <GalleryGridVideo
          key={`${src}-${indexInGrid}`}
          gridSrc={src}
          ...
          videoObjectClassName={
            videoObjectClassName.length > 0 ? videoObjectClassName : undefined
          }
  ...
  /**
   * Та-Чингоуза (summer-1): 15 кадров после hero/preface (без `hero.webp`, `shiroki2.webp`, `cave.webp`).
   * Порядок `slice(2)`: clip1–clip7, voda, pesochek, dlinni, most, saponi, stoit, stone.
   * Сверху `clip3` 2×2; `clip1`/`clip2` bento (`clip5` 1×1); `voda` 2×2; `clip6` bento; `most` 2×2, под ним `saponi`/`stoit` 1×2; внизу `stone`.
   */
  if (
    layoutVariant === 'tachingouza' &&
    images.length === 15 &&
    images[0] === TOUR_SUMMER_1_CLIP1_GRID_WEBM &&
    images[3] === TOUR_SUMMER_1_CLIP3_GRID_WEBM &&
    images[9] === TOUR_SUMMER_1_CLIP7_GRID_WEBM
  ) {
    const tachingouzaBentoRow = (left: number, rightTop: number, rightBottom: number) => (
      <div className="grid min-w-0 grid-cols-2 gap-gallery-gap">
        {renderTileButton(images[left], left, GALLERY_GRID_BENTO_TALL_LEFT)}
        {renderTileButton(images[rightTop], rightTop, GALLERY_GRID_BENTO_RIGHT_TOP_SQUARE)}
        {renderTileButton(images[rightBottom], rightBottom, GALLERY_GRID_BENTO_RIGHT_BOTTOM_SQUARE)}
    ...
    return (
      <div className="flex flex-col gap-gallery-gap">
        ...
        {tachingouzaBentoRow(0, 4, 9)}
        {tachingouzaBentoRow(1, 5, 6)}
        <div className="grid min-w-0 grid-cols-2 gap-gallery-gap">
          {renderTileButton(images[2], 2, GALLERY_GRID_FULL_WIDTH_TILE_2X2)}
        </div>
        {tachingouzaBentoRow(7, 8, 10)}
        <div className="flex flex-col gap-gallery-gap">
          <div className="grid min-w-0 grid-cols-2 gap-gallery-gap">
            {renderTileButton(images[11], 11, GALLERY_GRID_FULL_WIDTH_TILE_2X2)}
          </div>
          <div className="grid min-w-0 grid-cols-2 gap-gallery-gap">
            {renderTileButton(images[12], 12, GALLERY_GRID_BENTO_TALL_LEFT)}
            {renderTileButton(images[13], 13, GALLERY_GRID_BENTO_TALL_RIGHT)}
          </div>
        </div>
        <div className="grid min-w-0 grid-cols-2 gap-gallery-gap">
  ...
  /**
   * Остров Шкота (spring-11): 12 кадров после hero/preface.
   * Порядок `slice(2)`: clip1, view2, view3, clip2, climb, view4, clip3, view5, view6, wow, scallops, earchin.
   */
  if (
    layoutVariant === 'shkota' &&
    images.length === 12 &&
    images[0] === TOUR_SPRING_11_CLIP1_GRID_WEBM
  ) {
    return (
      <div className="flex flex-col gap-gallery-gap">
        <div className="grid min-w-0 grid-cols-2 gap-gallery-gap">
        ...
        <div className="grid min-w-0 grid-cols-2 gap-gallery-gap">
          {renderTileButton(images[4], 4, GALLERY_GRID_BENTO_LEFT_TOP_SQUARE)}
          {renderTileButton(images[5], 5, GALLERY_GRID_BENTO_LEFT_BOTTOM_SQUARE)}
          {renderTileButton(images[3], 3, GALLERY_GRID_BENTO_TALL_RIGHT)}
        </div>
        <div className="grid min-w-0 grid-cols-2 gap-gallery-gap">
          {renderTileButton(images[6], 6, GALLERY_GRID_FULL_WIDTH_TILE_2X2)}
        </div>
        ...
        <div className="grid min-w-0 grid-cols-2 gap-gallery-gap">
          {renderTileButton(images[9], 9, GALLERY_GRID_FULL_WIDTH_SQUARE)}
        </div>
        <div className="grid min-w-0 grid-cols-2 gap-gallery-gap">
          {renderTileButton(images[10], 10, GALLERY_GRID_SQUARE_TILE)}
   ...
   * Порядок `slice(2)`: clip6, clip1, clip3, view7, rocks, clip5,
   * sosna2, dve-sosna, summit-view, astafiev-bay.
   * `view7` (2×2), bento `summit` | `rocks` + `clip3`, широкий `clip5` (был слот sosna),
   * две «сосны», финальный bento: слева вертикально `clip1`, справа сверху `clip6`, справа снизу Астафьева.
   */
  if (
    layoutVariant === 'gamova' &&
    images.length === 10 &&
    images[0] === TOUR_SPRING_13_CLIP6_GRID_WEBM &&
    images[1] === TOUR_SPRING_13_CLIP1_GRID_WEBM &&
    images[2] === TOUR_SPRING_13_CLIP3_GRID_WEBM &&
    images[5] === TOUR_SPRING_13_CLIP5_GRID_WEBM &&
    images[8] === TOUR_SPRING_13_SUMMIT_VIEW_IMAGE &&
    images[9] === TOUR_SPRING_13_ASTAFIEV_BAY_IMAGE
  ) {
    return (
      <div className="flex flex-col gap-gallery-gap">
        <div className="grid min-w-0 grid-cols-2 gap-gallery-gap">
        ...
        <div className="grid min-w-0 grid-cols-2 gap-gallery-gap">
          {renderTileButton(images[5], 5, GALLERY_GRID_FULL_WIDTH_TILE_2X2)}
        </div>
        <div className="grid min-w-0 grid-cols-2 gap-gallery-gap">
        ...
        <div className="grid min-w-0 grid-cols-2 gap-gallery-gap">
          ...
          {renderTileButton(images[9], 9, GALLERY_GRID_BENTO_RIGHT_BOTTOM_SQUARE)}
        </div>
      </div>
    );
  }
