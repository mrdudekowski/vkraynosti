...
/** Горизонтальный gap между фото и bio на ≥650px; синхронно с `gap.team-hero-desktop`. */
export const TEAM_HERO_DESKTOP_COLUMN_GAP = '1.5rem' as const;

/** Максимальная ширина слайда team-hero на ≥650px; синхронно с `maxWidth.team-hero-slide`. */
export const TEAM_HERO_SLIDE_MAX_WIDTH = 'min(100%, 56rem)' as const;

/** Единый вертикальный отступ team-hero на mobile (≤649px). */
export const TEAM_HERO_MOBILE_VERTICAL_GAP = '2rem' as const;

/** Gap между article Элины и Ярослав на ≤649px; синхронно с `spacing.team-hero-members-stack-mobile`. */
...
/** Нижний padding первого блока на team-hero-desktop (650–767px) — под staircase SM и stack 3rem. */
...
/** Gap между блоками на team-hero-desktop (650px)–md; синхронно с `spacing.team-hero-members-stack`. */
...
/**
 * Подъём фото второго блока на team-hero-desktop (650–767px): умеренный clamp, чтобы не наезжать на bio Элины.
 * При stack 3rem + pb 2rem даёт ~2rem видимого зазора при cap 3rem.
 */
export const TEAM_HERO_STAIRCASE_OFFSET_SM = 'clamp(2rem, 10vw, 3rem)' as const;
